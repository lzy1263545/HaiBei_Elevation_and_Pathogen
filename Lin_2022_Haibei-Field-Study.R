...
###############: Auto-model-selection for a full model
#####: For pathogen load (PL)
ms.data <- Field_study_data %>% select(1:7,10:12)
ms.data <- as.data.frame(scale(ms.data)) %>% mutate(lat = Field_study_data$lat,
                                                    lon = Field_study_data$lon,
                                                    Elevation = Field_study_data$Elevation)
ms.model_PL <- lme(data = ms.data,PL~Ele+Soil_PCA1+AB+BB+Ev+p+SR,
          correlation = corSpher(1,~lat+lon),
          random = ~ 1 | Elevation, method = "ML",na.action = "na.fail") #: construct the full model
ms.result_PL <- dredge(ms.model_PL, extra = c("R^2", F = function(x) #: model selection
  summary(x)$fstatistic[[1]]))
#write.csv(ms.result_PL,"ms.result_PL.csv")

par(mar = c(3,5,6,4))
plot(ms.result_PL, labAsExpr = TRUE)
mod.ave_PL <- model.avg(ms.result_PL,subset = delta < 4) #: averaging the model
summary(mod.ave_PL) #: report the results
confint(mod.ave_PL)
coef_PL <- coef(mod.ave_PL)
CI_PL <- confint(mod.ave_PL)
ms.plot_PL <- as.data.frame(cbind(CI_PL,coef_PL)) %>% mutate(variable = rownames(ms.plot_PL)) %>% filter(variable != "(Intercept)")
colnames(ms.plot_PL) <- c("lb","ub","mean","variable")

#####: For soil fungal pathogen OTU richness (sfpOTUs)
ms.model_sfpOTUs <- lme(data = ms.data,sfpOTUs~Ele+Soil_PCA1+AB+BB+Ev+p+SR,
                   correlation = corSpher(1,~lat+lon),
                   random = ~ 1 | Elevation, method = "ML",na.action = "na.fail") #: construct the full model
ms.result_sfpOTUs <- dredge(ms.model_sfpOTUs, extra = c("R^2", F = function(x) #: model selection
  summary(x)$fstatistic[[1]]))
#write.csv(ms.result_sfpOTUs,"ms.result_sfpOTUs.csv")

par(mar = c(3,5,6,4))
plot(ms.result_sfpOTUs, labAsExpr = TRUE)
mod.ave_sfpOTUs <- model.avg(ms.result_sfpOTUs,subset = delta < 4) #: averaging the model
summary(mod.ave_sfpOTUs) #: report the results
confint(mod.ave_sfpOTUs)
coef_sfpOTUs <- coef(mod.ave_sfpOTUs)
CI_sfpOTUs <- confint(mod.ave_sfpOTUs)
ms.plot_sfpOTUs <- as.data.frame(cbind(CI_sfpOTUs,coef_sfpOTUs)) %>% mutate(variable = rownames(ms.plot_sfpOTUs)) %>% filter(variable != "(Intercept)")
colnames(ms.plot_sfpOTUs) <- c("lb","ub","mean","variable")

#####: For soil fungal pathogen relative abundance (sfpRA)
ms.model_sfpRA <- lme(data = ms.data,sfpRA~Ele+Soil_PCA1+AB+BB+Ev+p+SR,
                        correlation = corSpher(1,~lat+lon),
                        random = ~ 1 | Elevation, method = "ML",na.action = "na.fail") #: construct the full model
ms.result_sfpRA <- dredge(ms.model_sfpRA, extra = c("R^2", F = function(x) #: model selection
  summary(x)$fstatistic[[1]]))
#write.csv(ms.result_sfpRA,"ms.result_sfpRA.csv")

par(mar = c(3,5,6,4))
plot(ms.result_sfpRA, labAsExpr = TRUE)
mod.ave_sfpRA <- model.avg(ms.result_sfpRA,subset = delta < 4) #: averaging the model
summary(mod.ave_sfpRA) #: report the results
confint(mod.ave_sfpRA)
coef_sfpRA <- coef(mod.ave_sfpRA)
CI_sfpRA <- confint(mod.ave_sfpRA)
ms.plot_sfpRA <- as.data.frame(cbind(CI_sfpRA,coef_sfpRA)) %>% mutate(variable = rownames(ms.plot_sfpRA)) %>% filter(variable != "(Intercept)")
colnames(ms.plot_sfpRA) <- c("lb","ub","mean","variable")
...
#####: Plot the effect size for full model selection
theme_set(theme_test())
mytheme = theme(panel.border = element_rect(size = 1,color = "black"),
                plot.subtitle = element_text(size = 14,family = "serif"),
                axis.text.x = element_text(size = 14,face = "bold",family = "serif"),
                axis.text.y = element_text(size = 14,face = "bold.italic",family = "serif"),
                axis.title.x = element_text(size = 20,face = "bold",family = "serif"),
                axis.title.y = element_text(size = 20,face = "bold",family = "serif"),
                legend.position = "none")

plot.ms_PL <- ggplot(data = ms.plot_PL)+
  geom_point(aes(x = mean,y = variable,color = variable),shape = 16,size = 8)+
  geom_errorbar(aes(xmin = lb,xmax = ub,y = variable,color = variable),width = 0.25,size = 1,alpha = 0.75)+
  geom_vline(xintercept = 0,size = 0.5,linetype = 2)+
  scale_color_manual(values = c("gray80","gray80","gray80","#3C5488FF","#E64B35FF","gray80","gray80"))+
  xlab("Effect size")+
  ylab("")+
  labs(subtitle = "Pathogen load")+
  mytheme
plot.ms_PL  

plot.ms_sfpOTUs <- ggplot(data = ms.plot_sfpOTUs)+
  geom_point(aes(x = mean,y = variable,color = variable),shape = 16,size = 8)+
  geom_errorbar(aes(xmin = lb,xmax = ub,y = variable,color = variable),width = 0.25,size = 1,alpha = 0.75)+
  geom_vline(xintercept = 0,size = 0.5,linetype = 2)+
  scale_color_manual(values = c("gray80","gray80","#3C5488FF","gray80","gray80","#3C5488FF","gray80"))+
  xlab("Effect size")+
  labs(subtitle = "Soil fungal pathogen OTU richness")+
  ylab("")+
  mytheme
plot.ms_sfpOTUs

plot.ms_sfpRA <- ggplot(data = ms.plot_sfpRA)+
  geom_point(aes(x = mean,y = variable,color = variable),shape = 16,size = 8)+
  geom_errorbar(aes(xmin = lb,xmax = ub,y = variable,color = variable),width = 0.25,size = 1,alpha = 0.75)+
  geom_vline(xintercept = 0,size = 0.5,linetype = 2)+
  scale_color_manual(values = c("gray80","gray80","gray80","gray80","gray80","gray80","gray80"))+
  xlab("Effect size")+
  labs(subtitle = "Soil fungal pathogen relative abundance")+
  ylab("")+
  mytheme
plot.ms_sfpRA 

ggarrange(plot.ms_PL,plot.ms_sfpOTUs,plot.ms_sfpRA,ncol = 3,legend = "none",labels = "AUTO")
ggsave("Model selection.pdf",height = 8,width = 14)
